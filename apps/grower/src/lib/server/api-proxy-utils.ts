...
export function isTokenExpired(expiresAt: number, bufferSeconds = 30): boolean {
  ...
  const isExpired = now >= expirationThreshold;
...
export const BLOCKED_REQUEST_HEADERS = new Set([
  ...
  "upgrade",
  "te",
  "cookie", // Don't forward cookies - we use Bearer token auth
  "trailer",
  "transfer-encoding",
  "proxy-authorization",
  "proxy-authenticate",
...
export async function prepareRequestBody(
  ...
  // Check if body is actually empty before attempting to parse
  // This prevents hanging on empty bodies with content-type set
  ...
  if (contentType.includes("application/json")) {
    try {
      ...
      // Only stringify if we got actual data
      if (jsonBody === null || jsonBody === undefined) {
        return undefined;
      }
      return JSON.stringify(jsonBody);
    } catch (_) {
      // If JSON parsing fails, return undefined for empty body
      return undefined;
    }
