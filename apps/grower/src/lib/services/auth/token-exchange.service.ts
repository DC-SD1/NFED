...
export class TokenExchangeService {
  ...
  async handleUserNotFoundWithRegistration(
    ...
    try {
      ...
      const registrationResponse = await api.client.POST("/users/register", {
        body: {
          email: clerkUser.email,
          authId: clerkUser.sub,
          firstName: clerkUser.firstName || "",
          lastName: clerkUser.lastName || "",
          phoneNumber: null,
          roles: [ROLES.FARM_OWNER], // Default role for new users
        },
      });

      if (registrationResponse.error) {
        const regError = registrationResponse.error.errors?.[0];
        logger.error("Fallback registration failed", {
          error: registrationResponse.error,
          errorCode: regError?.code,
          traceId: registrationResponse.error.traceId,
        });
      ...
      if (retryResponse.error) {
        logger.error("Login failed even after successful registration", {
          ...
          registeredUserId: registrationResponse.data?.userId,
        });
