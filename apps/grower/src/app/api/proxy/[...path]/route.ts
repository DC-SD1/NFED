...
async function handler(
  ...
  try {
    ...
    // Remove content-type header for requests without a body
    if (
      request.method === "GET" ||
      request.method === "HEAD" ||
      body === undefined
    ...
    // Debug token details
    if (accessToken) {
      // Parse JWT to check expiration (without verifying signature)
      try {
        ...
        if (tokenParts.length === 3 && tokenParts[1]) {
          ...
          logger.info("Token debug info", {
            ...
            isExpired: exp ? exp < now : "no-exp-claim",
            timeUntilExpiry: exp
        ...
        logger.warn("Could not parse token for debugging", {
          requestId,
    ...
    // Log the actual headers being sent to backend
    const headersObj: Record<string, string> = {};
    headers.forEach((value, key) => {
      // Don't log the full auth token for security, just confirm it exists
      if (key.toLowerCase() === "authorization") {
        headersObj[key] = `Bearer ${value.substring(7, 27)}...`;
      } else {
