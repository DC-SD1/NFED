...
import type { NewbieData } from "@/lib/stores/sign-up-store";
...
export function CommitmentContent() {
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      if (startError) {
        throw new Error(e("start_error"));
      }
      const responses: Record<string, string | string[] | NewbieData> = {
        farmingExperience: "newbie",
      ...
      if (submitError) {
        throw new Error(e("submit_error"));
      }

      router.push("/onboarding/newbie/complete");
    } catch (err) {
