...
import type { FarmingContract } from "@/components/dashboard/order-details-cards/farm-agreement-card";
...
export default function Page() {
  ...
  const mockContractId = "92367e46-f108-4912-9b84-bcebd21deaf1"; // hardcoded for now

  const { data } = api.useQuery("get", "/contracts/{ContractId}", {
    params: {
      path: {
  ...
  const farmContractData: FarmingContract = data?.contractDetails
      ...
      <div className="px-2 py-6 md:items-center md:justify-center md:px-6 md:pt-0 lg:px-8">
        <div className="mx-auto w-full max-w-lg md:max-w-4xl lg:max-w-2xl">
          ...
          <FarmingAgreementCard
            crop={farmContractData?.crop}
            area={`${farmContractData?.area} acres`}
            contractValue={
              farmContractData?.contractValue
                ? `GHS${farmContractData?.contractValue}`
                : undefined
            }
            inputSupport={`${farmContractData?.inputSupport}`}
            deliveryDate={farmContractData?.deliveryDate}
            paymentTerms={farmContractData?.paymentTerms}
            qualityStandards={farmContractData?.qualityStandards}
            isAgreed={isAgreed}
            onAgreementChange={() => setIsAgreed((prev) => !prev)}
            nextPageUrl="/farm-owner/farm-grow/focus-order/sign"
          />
          ...
          <div className="mt-8 flex items-center justify-center">
            <Button
              ...
              onClick={() =>
                router.push(
                  `/farm-owner/farm-grow/focus-order/sign?contractId=${mockContractId}`,
                )
              }
            >
              {t("sign")}
