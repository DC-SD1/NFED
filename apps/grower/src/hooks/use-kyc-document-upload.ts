  ...
  const uploadFunction = async (
    ...
    try {
      ...
      if (response.error) {
        ...
        throw new Error(firstError?.message || "KYC document upload failed");
      }

      if (!response.data) {
        throw new Error("No data returned from KYC document upload");
      }

      onProgress(100);
      logger.info("KYC document upload successful", {
        fileName: file.name,
      });
