...
export function AppSidebar({ locale = "en" }: AppSidebarProps) {
  ...
  const getActiveChildId = (item: NavigationItem) => {
    if (!item.children) return null;

    const pathWithoutLocale = pathname.replace(/^\/[a-z]{2}/, "");
    const activeChild = item.children.find(
      (child) =>
        pathWithoutLocale.startsWith(child.href) ||
  ...
  return (
    <V0Sidebar variant="inset">
      <V0SidebarHeader className="h-42 flex flex-col items-center justify-start gap-8 p-4 py-1">
        <CF className="flex size-14 items-center justify-center rounded-2xl bg-primary" />
          ...
          {sidebarSections.map((section) => (
            <div key={section.id} className="flex flex-col gap-2">
              {section.items.map((item) => {
                // Use special expandable component ONLY for wallet item
                if (
                  item.id === "wallet" &&
                  item.children &&
                  ...
                  return (
                    <V0SidebarNavItemWithChildren
                      ...
                      activeChildId={getActiveChildId(item)}
                      locale={locale}
                    />
                  );
                }

                // Regular nav item for all other items (including those with children)
                return (
                  <V0SidebarNavItem
                    key={item.id}
