    ...
    try {
      ...
      if (tokenResponse.error) {
        ...
        if (
          ...
          const registrationResponse = await api.client.POST(
            "/admin/register",
            {
              body: {
                ...
                phoneNumber: '0', // OAuth users might not have phone
              },
            },
          );
          ...
          if (response.error) {
            const regError = response.error;
            const firstRegError = regError.errors?.[0];
            const generalErrorMessages = (regError.errors?.generalErrors || []).join("\n");
            const message = regError?.statusCode === 400 ? generalErrorMessages : firstRegError?.message || "Registration failed";
            const errorCode = regError?.statusCode === 400 ? 'VALIDATION_ERROR' : firstRegError?.code ?? 'USER_NOT_FOUND';

            logger.error("OAuth admin registration failed", {
              error: regError,
              errorCode,
              errors: JSON.stringify(regError),
              messages: regError.errors.generalErrors.join('\n'),
            });
          ...
          logger.info("OAuth admin registered successfully", {
      ...
      const mappedRoles = mapLegacyRoles(tokenData.roles || [ROLES.ADMIN]);
