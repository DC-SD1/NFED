...
export default function CenterManagerTable<T>({
  data,
  paginateData,
  toolBar,
  isFiltering = false,
  ...
  const ACTION_CONFIG: TableActionItem<CenterManager>[] = [
    {
  ...
  const columns: ColumnDef<CenterManager>[] = [
    {
      id: "select",
      header: ({ table }) => (
        <Checkbox
      ...
      {!isFiltering && data.length === 0 ? (
        <div className={"flex h-64 flex-col items-center justify-center"}>
          <ContentEmptyState
            ...
            title={"No manager to display"}
            message={
              "Managers linked to fulfilment center will appear here once added."
            }
          />
        </div>
      ) : (
        <DataTableComponent
          ...
          emptyStateTitle={"No manager found"}
