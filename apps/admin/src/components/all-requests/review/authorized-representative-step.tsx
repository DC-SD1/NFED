...
export default function AuthorizedRepresentativeStep() {
      ...
      {authRep.length > 0 ? (
        authRep.map((rep, index) => (
          <ApprovalTileItem
            key={index}
            title={""}
            value={humanizeLabel(rep?.file_name || "N/A")}
            onViewClick={() =>
              handleViewImage(rep?.file_url, rep?.file_name || "")
            }
            onApproveClick={() => {
              handleUpdateDocument(rep?.file_name ?? "", "Accepted");
            }}
            onDeclineClick={() => {
              setDocumentStatus("auth_rep", rep?.file_name ?? "", "Required");
            }}
            status={(rep?.document_status || "").toLowerCase()}
            ...
            onDeclineCancelClick={() => {
              setDocumentStatus("auth_rep", rep?.file_name ?? "", "Pending");
            }}
            onDeclineSaveClick={() => {
              handleUpdateDocument(
                rep?.file_name ?? "",
                "Declined",
                rep?.status_error_message,
              );
            }}
            onChangeClick={() => {
              setDocumentStatus("auth_rep", rep?.file_name ?? "", "Pending");
            }}
            reason={rep?.status_error_message ?? ""}
            isLoading={isPending}
