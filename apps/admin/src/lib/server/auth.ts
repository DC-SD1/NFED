...
export async function getServerAuth(): Promise<UserMetadata | null> {
  try {
    ...
    if (!isValid) {
      // Cookie recovery is now handled by middleware
      // If we reach here with invalid cookies, it means:
      // 1. This is a public route, or
      // 2. Middleware recovery failed, or
      // 3. The user is truly unauthenticated
