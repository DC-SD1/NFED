...
export function LoginForm() {
  ...
  // Show session expired notice if redirected with reason
  useEffect(() => {
    const reason = searchParams?.get("reason");
    if (reason === "session_expired") {
      // Use localized error message key
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              ...
              className="inline-flex items-center gap-1 text-sm font-semibold text-[#4B908B] hover:text-[#4B908B]/80"
            >
              {t("recoverPassword")}
