  ...
  try {
    ...
    try {
      ...
      logger.info("Backend refresh response received", {
        ...
        expiresAtLength:
          typeof refreshResult?.expiresAt === "string"
            ? (refreshResult.expiresAt as string).length
            : typeof refreshResult?.expiresAt === "number"
              ? refreshResult.expiresAt.toString().length
              : 0,
      });

      // Validate response structure
      const validationResult =
        TokenRefreshResponseSchema.safeParse(refreshResult);
      ...
      if (newRefreshToken) {
        ...
        if (existingUserMetadata) {
          ...
          if (currentCookies) {
            ...
            for (const pair of cookiePairs) {
              ...
              if (
                (name === AUTH_COOKIE_NAMES.USER_METADATA ||
                  name === `__Secure-${AUTH_COOKIE_NAMES.USER_METADATA}`) &&
                value
              ) {
                try {
        ...
        const { hasAuthCookies } = await import(
            ...
            for (const pair of cookiePairs) {
              ...
              if (
                ...
                try {
                  ...
                  logger.info(
                    "Parsed user metadata from raw cookies for access token update",
                    {
                      userId: userMetadata.userId,
                      email: userMetadata.email,
                      hasRoles: userMetadata.roles.length > 0,
