...
import type { AuthStoreApi } from "@/lib/stores/auth/auth-store-ssr";
 ...
 *
 * Note: This now requires the auth store instance to be passed in
 * since we're using the context pattern instead of a global store
 */
...
/**
 * Initialize cross-tab sync with the auth store instance
 * Call this from a component that has access to the auth store context
 */
export function initializeCrossTabSync(authStore: AuthStoreApi): () => void {
    ...
    authChannel.onmessage = (event) => {
      try {
        if (event.data.type === "AUTH_STATE_UPDATE") {
          ...
          authStore.setState({ ...currentState, ...authState });
        }
      } catch (error) {
