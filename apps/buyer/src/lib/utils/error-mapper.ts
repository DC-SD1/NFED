...
export function mapErrorToLocaleKey(error: unknown): string {
  ...
  logger.info("Mapping error to locale key", {
    ...
    errorErrors: (error as any)?.errors || "N/A",
  });
  ...
  if (typeof error === "object" && error !== null) {
    ...
    // Priority 1: Check for nested error structures (Clerk or openapi-fetch)
    if (
      errorObj.errors &&
      ...
      logger.info("Found errors array", { firstError });

      if (firstError?.code && typeof firstError.code === "string") {
        // Check Clerk-specific codes first
        if (CLERK_ERROR_MAP[firstError.code]) {
          logger.info("Found Clerk error code in errors array", {
        ...
        if (SHARED_KERNEL_ERROR_MAP[firstError.code]) {
          logger.info("Found API error code in errors array", {
            code: firstError.code,
          });
          return (
    ...
    if (errorObj.errorCode && SHARED_KERNEL_ERROR_MAP[errorObj.errorCode]) {
      logger.info("Found API error code on top level", {
    ...
    if (errorObj.code && typeof errorObj.code === "string") {
      if (CLERK_ERROR_MAP[errorObj.code]) {
        logger.info("Found Clerk error code on top level", {
          code: errorObj.code,
        });
    ...
    if (errorObj.status && HTTP_STATUS_ERROR_MAP[errorObj.status]) {
      logger.info("Found HTTP status error", { status: errorObj.status });
      return (
    ...
    if (errorObj.message && typeof errorObj.message === "string") {
      ...
      logger.info("Checking error message content", { message });
      ...
      if (
        ...
        message.includes("identifier_already_exists")
      ) {
        logger.info(
          "Found 'already exists' in message, returning email_already_exists",
        );
        return "auth.errors.email_already_exists";
      }
      if (message.includes("password") && message.includes("strong")) {
        logger.info(
          "Found password strength issue in message, returning password_too_weak",
        );
        return "auth.errors.password_too_weak";
      }
      if (message.includes("invalid") && message.includes("email")) {
        logger.info("Found invalid email in message, returning invalid_email");
