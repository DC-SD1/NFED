...
import { useAuthActions, useAuthTokens } from "../stores/auth/auth-store-ssr";
...
export function useApiClient(): ApiClient {
  ...
  const { actions: authActions } = { actions: useAuthActions() } as const;
  ...
  return useMemo(
    () =>
      createApiClient({
        ...
        onLogout: async () => {
          logger.info("API client logout triggered - handling with auth store");
          try {
            await authActions.logout(signOut, router, locale, {
              reason: "session_expired",
            });
          } catch (e) {
            logger.error("Logout handling failed", e);
          }
        },
    ...
    [expiresAt, refreshTokens, authActions, signOut, router, locale],
  );
}
