...
export const useGanttDragging = () => {
  const isDragging = useGanttStore((state) => state.isDragging);
  const setDragging = useGanttStore((state) => state.setDragging);
  return [isDragging, setDragging] as const;
};

export const useGanttScrollX = () => {
  const scrollX = useGanttStore((state) => state.scrollX);
...
export const GanttProvider: FC<GanttProviderProps> = ({
  ...
  // Create throttled scroll handler with proper typing
  const throttledScrollHandler = useMemo(
    () => {
      ...
      return throttle(handler, 100) as () => void;
    },
  ...
  const scrollToFeature = useCallback(
    (feature: GanttFeature) => {
      const scrollElement = scrollRef.current;
      if (!scrollElement || !timelineData[0]) {
        return;
      }
