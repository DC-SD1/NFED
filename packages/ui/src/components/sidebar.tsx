...
>(({ className, onClick, sidebarIcon, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      {...props}
    >
      {sidebarIcon ? sidebarIcon : <PanelLeft />}
...
// V0 Components - Adding the missing components from v0
...
>(({ className, icon, label, isActive, href, ...props }, ref) => {
  ...
  const itemContent = (
    <div
      className={cn(
        "flex min-h-[70px] flex-col items-center justify-center gap-1 rounded-2xl p-3 transition-colors",
        isActive
...
>(({ className, icon: Icon = PanelLeft, ...props }, ref) => {
